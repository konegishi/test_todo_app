...
interface TaskInputFormProps {
  ...
  /** タスク名 */
  name?: string;
  onClickHandler: (user: User, taskText: string) => void;
}
...
const TaskInputForm: React.FC<TaskInputFormProps> = (props) => {
  ...
  // const { fuga, setFuga } = useContext(FugaContext);

  /**
   * ファームのタスク追加ボタンのhandler
   */
  const handleOnClickAddButton = () => {
    props.onClickHandler(props.user, inputRef.current.value);
    // addTodo(props.user, inputRef.current.value);
  ...
  return (
    <React.Fragment>
      ...
      {/* タスク入力フォーム */}
      <div className='shadow absolute bottom-0 left-0 right-0 z-40 overflow-y-auto overflow-x-hidden h-auto items-center flex-1 rounded-t bg-white p-2 sticky'>
